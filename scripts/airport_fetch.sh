#!/usr/bin/env bash
set -euxo pipefail
rm -rf out airport-fetch-output
...
(async () => {
  ...
  fs.writeFileSync('out/page-html.html',await page.content());

  let r=await context.request.get(endpoint,{
    headers:{referer:project,accept:'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'},
    timeout:120000,maxRedirects:20
  });
  ...
  if(body.length>=4&&body[0]===0x50&&body[1]===0x4b&&body[2]===0x03&&body[3]===0x04){
    fs.writeFileSync('out/Scarx28-Regional-Airport-Project.zip',body);
    await browser.close(); return;
  }
  fs.writeFileSync('out/browser-response.html',body);

  const link=page.locator('a:has-text("Download Minecraft Map")').first();
  if(await link.count()){
    const href=await link.getAttribute('href');
    fs.writeFileSync('out/download-href.txt',String(href));
    try{
      const dp=page.waitForEvent('download',{timeout:60000});
      await link.click({timeout:30000});
      const dl=await dp;
      await dl.saveAs('out/Scarx28-Regional-Airport-Project.zip');
      await browser.close(); return;
    }catch(e){
      fs.writeFileSync('out/click-error.txt',String(e));
      fs.writeFileSync('out/after-click-url.txt',page.url());
    }
  } else {
    fs.writeFileSync('out/link-missing.txt','Download Minecraft Map link not found in rendered page.');
  }
  await browser.close();
})().catch(e=>{fs.writeFileSync('out/browser-fatal.txt',String(e && e.stack || e));});
JS
  node fetch-airport.js 2>out/node-error.txt || true
fi
